...
import choreo.oldChoreo.Agent.{Memory, Participant}
import choreo.oldChoreo.Choreography._
import choreo.oldChoreo.GuardedCommand._
import choreo.oldChoreo.syntax.GlobalContext._
import choreo.oldChoreo.syntax.Program._
...
//    yield (choreo, channels)
  ...
  def isClosed(ch:Channel,pos:Position):Interpret[Unit] = for
    ...
    _ <- if notUsed.isEmpty then ().pure[Interpret]
  ...
  def checkWellDefinedGC(gc: GuardedCommand,pos:Position): Interpret[Unit] =
    ...
    if senders.subsetOf(gets) then ().pure[Interpret]
  ...
  def getAgent(v: Variable): Interpret[Agent] =
    for
      st <- getSt
  ...
  def canMatch(i: Interaction, ch: Channel,pos:Position): Interpret[Unit] =
    if ((i.senders.size != ch.senders.size) ||
      ...
      (i.memories.size > ch.memories.size)) then
