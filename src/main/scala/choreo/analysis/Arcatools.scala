...
object Arcatools:
  ...
  val widgets: List[(Widget,String)] = List(
    Visualize(ChorMerView,id) ->
      "Sequence Diagram",
    Simulate(ChorDefSOS,ChorTxtView,id) ->
      "Simulate Choreo (Default)",
    Simulate(ChorBasicSOS,ChorTxtView,id) ->
      "Simulate Choreo (Basic)",
    Simulate(ChorManyTausSOS,ChorTxtView,id) ->
    ...
    Simulate(GlobalPom,PomMerView,chor2pom) ->
      "Simulate Pomset",
    Visualize(CollView(PomMerView), c => proj.PomDefProj.allProj(chor2pom(c))) ->
