...
object Lucs :
  ...
  def dista(c1:Choreo,c2:Choreo): Boolean = (c1,c2) match
    ...
    case (DChoice(c1a, c1b),_) => dista(c1a,c2) || dista(c1b,c2)
    case (_,DChoice(c2a, c2b)) => dista(c1,c2a) || dista(c1,c2b)
    ...
    case (_,_) => error(s"case untreated: $c1, $c2")
  ...
  def distp(c1:Choreo, c2:Choreo): Boolean = (c1,c2) match
    ...
    case (DChoice(c1a, c1b),_) => distp(c1a,c2) || distp(c1b,c2)
    case (_,DChoice(c2a, c2b)) => distp(c1,c2a) || distp(c1,c2b)
    case (Loop(_),_) => error("[dista] loops nos handled")
    case (_,Loop(_)) => error("[dista] loops nos handled")
    case (End,End) => false
    case (End,_) => true // ?
  ...
  def realisableLuc(c:Choreo): Boolean = c match
    ...
    case Tau => true
    case _: Action => true
    case _ => error(s"Case $c:${c.getClass} not supported for reliseability (Luc's)")
