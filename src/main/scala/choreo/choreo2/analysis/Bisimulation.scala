...
//              newError = err.msg
  ...
  private def collectMore[G:LTS,L:LTS](g:G,l:L,t:List[Action]): Either[List[String], S[G,L]] =
    var more:S[G,L] = none
      // for every g-a->g2
    for (a,g2)<- g.trans do
//      println(s"\n### doing $a\n[Sim] G $g ")
...
//            println(s"[B] new mbMatch: $mbMatch")
  ...
  // utils
  private def one[G:LTS,L:LTS](g:G,l:L,t:List[Action]):S[G,L] = Set(Map((g,l)->t))
  private def none[G:LTS,L:LTS]:S[G,L] = Set(Map())
  private def add[G:LTS,L:LTS](m:S[G,L],g:G,l:L,t:List[Action]):S[G,L] = m.map(_+((g,l)->t))
  private def and[G:LTS,L:LTS](x:S[G,L],y:S[G,L]): S[G,L] =
    for m1<-x; m2<-y yield m1++m2
  private def or[G:LTS,L:LTS](x:S[G,L],y:S[G,L]): S[G,L] =
    x ++ y
  private def ors[G:LTS,L:LTS](x:Set[S[G,L]]): S[G,L] =
    x.flatten
  private def swap[G:LTS,L:LTS](x:S[G,L]):S[L,G] =
